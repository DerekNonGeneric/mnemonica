...
const tests = (opts) => {
	...
	describe('Check Environment', () => {
		...
		describe('additional props test', () => {
			...
			expect(props_alt.ownProp).is.equal(123);
			expect(props_alt.__self__).is.equal(props_int.__self__);
		...
		describe('missing instance props test', () => {
			...
			expect(getProps(new Number(7))).is.equal(undefined);
			expect(getProps(new Boolean(true))).is.equal(undefined);
			expect(getProps(new String('123'))).is.equal(undefined);
